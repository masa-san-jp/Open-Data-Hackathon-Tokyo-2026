...
## ADR-0011: M11Bでは暫定250mメッシュ代表点を徒歩計算の起点候補として表示する

- status: accepted（M11BのUI表示に限定）
- context: 徒歩条件を表示するには起点の説明が必要だが、青梅市の正式な人口メッシュ、道路ネットワーク、標高DEM、徒歩モデルは未採用である。起点なしの条件UIは利用者に誤解を与える。
- decision: 行政区域内にローカル座標の250m正方グリッドを生成し、各セル中心を「暫定起点」として表示する。利用者はセルを選択できるが、徒歩時間・到達圏・政策判定の計算には使用しない。
- alternatives: 起点を市役所・地図中心・施設点に固定する（根拠がなく不採用）、起点レイヤーを表示しない（条件UIの意味が不明になるため不採用）、正式な人口メッシュを先に実装する（M12/M13のデータ採用・人間確認が未完了のため保留）。
- consequences: 起点の所在と暫定性を画面で確認できる。正式な人口按分・道路・標高が採用されたら、このローカルグリッドを実分析メッシュへ置き換える必要がある。
- rollback: 起点レイヤーと選択UIを削除し、徒歩条件UIを正式計算実装まで非表示にする。
