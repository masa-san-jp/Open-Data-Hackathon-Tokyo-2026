...
| **P2** | 永続化と実データ | Hono + SQLite。銭湯マスタ投入。番台の＋1が保存され、翌日も残る | 進行中 |
...
- テスト例（必ずこの値で検証する）:
  ...
  予算A: price=550 addon=180 operatingDays=312
         ...
         cash=4000000 targetCash=4000000 yearsToCashTarget=1
    unitRevenue           = 730
...
| `T2-05` | `sessionId` からの `sequence` 採番 | `T2-03` | 未着手 | 同一セッション同日の3件が 1,2,3 になる |
| `T2-06` | `DailyCount` の日次集計（`unknown` を含む） | `T2-05` | 未着手 | 番台の＋1が `unknown` に入る |
| `T2-07` | `packages/api-client` の実装と、3アプリのストア差し替え | `T2-03` | 未着手 | UI を変えずに動く |
| `T2-08` | 銭湯マスタのシード | `T2-01` | 保留（U2待ち） | **`U2` の判断待ち。着手前に保留する** |
| `T2-09` | 休業日フラグと §4-7 の連続未達ロジック修正 | `T2-01` | 保留（U7待ち） | **`U7` の判断待ち** |
| `T2-10` | 銭湯管理UI（一覧・検索・新規登録・基本情報編集） | P1完了 | 完了 | 新規登録と登録済み基本情報の編集ができ、新規は予算未登録と表示される |
| `T2-11` | 現金目標を含む必要客数計画の仕様 | `T2-10` | 完了 | `targetCash` と年間現金積増を含む計算式・画面仕様が定義される |
| `T2-12` | 現金積増・年間必要売上のドメイン計算 | `T2-11` | 完了 | 純関数と境界値テストが通り、既存の必要客数計算へ反映される |
| `T2-13` | 必要客数計画UIとリアルタイム不足連動 | `T2-12` | 完了 | 管理で現金目標を変えると番台の目標と顧客アプリの提案順へ反映される |
| `T2-14` | 3画面のスマホ・タブレット操作性 | `T2-13` | 完了 | 390px幅で横スクロールがなく、主要操作が44px以上。adminの来客・目標・あと必要・達成率が横スクロールなしで見られる |
| `T2-15` | モダン和風カラーパレット | `T2-14` | 完了 | spec §8の配色を3画面へ統一適用し、黄色は主要操作、赤は警告だけに限定する |
| `T2-16` | 新パレットの視覚階層 | `T2-15` | 完了 | ヘッダー・KPI・必要客数・ルート面でmidnight/moss/hinokiの違いが視認でき、3画面の印象が統一される |
| `T2-17` | 富士山タイルのデザイン刷新 | `T2-16` | 完了 | spec §8の空色・水色・淡色タイルと角ゴシックを3画面へ適用し、業務情報の視認性とスマホ操作性を維持する |
| `T2-18` | React 3画面のCloudflare公開 | `T2-17` | 完了 | `/guest/` `/counter/` `/admin/` で最新ビルドを配信し、トップと旧URLから到達でき、公開URLのスモーク確認が通る。2026-08-27に `https://hashigoyu.machikado-qr.workers.dev` でHTTP 200と旧URLの308リダイレクトを確認済み |
