...
| `admin` | 管理 | PC | 全湯の閲覧、銭湯基本情報の登録・編集、予算の編集と確定 |
...
- 3画面とも `320px` 以上でページ全体の横スクロールを発生させない
- `480px` 以下では、主要なボタン・入力・選択・リンクのタップ領域を高さ `44px` 以上にする
- guest は入力条件とルートを1列表示し、地図リンクのタップ領域を `44px` 以上にする
- counter は番台操作を1列表示し、＋1／−1とタブを指で操作できる大きさにする
- admin のダッシュボードは銭湯ごとのカード表示に切り替え、来客・目標・あと必要・達成率を横スクロールなしで表示する。入力欄と確定・保存操作は高さ `44px` 以上にする
- `481px`〜`820px` の admin はKPIと必要客数計画を2列表示にする
